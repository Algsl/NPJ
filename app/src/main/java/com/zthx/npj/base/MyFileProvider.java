...
public class MyFileProvider extends FileProvider {
    ...
    @GuardedBy("sCache")
    private static HashMap<String, MyFileProvider.PathStrategy> sCache = new HashMap();
    private MyFileProvider.PathStrategy mStrategy;
    ...
    public static Uri getUriForFile(@NonNull Context context, @NonNull String authority, @NonNull File file) {
        MyFileProvider.PathStrategy strategy = getPathStrategy(context, authority);
    ...
    private static MyFileProvider.PathStrategy getPathStrategy(Context context, String authority) {
        HashMap var3 = sCache;
        synchronized(sCache) {
            MyFileProvider.PathStrategy strat = (MyFileProvider.PathStrategy)sCache.get(authority);
            if (strat == null) {
                try {
    ...
    private static MyFileProvider.PathStrategy parsePathStrategy(Context context, String authority) throws IOException, XmlPullParserException {
        MyFileProvider.SimplePathStrategy strat = new MyFileProvider.SimplePathStrategy(authority);
        @SuppressLint("WrongConstant") ProviderInfo info = context.getPackageManager().resolveContentProvider(authority, 128);
    ...
    static class SimplePathStrategy implements MyFileProvider.PathStrategy {
                ...
                do {
                    do {
                        if (!var4.hasNext()) {
                            ...
                            String rootPath1 = ((File)mostSpecific.getValue()).getPath();
                            if (rootPath1.endsWith("/")) {
                                path = path.substring(rootPath1.length());
                            } else {
                                path = path.substring(rootPath1.length() + 1);
                            }
